...
{
	public class DocumentSetApplicationOptionsCollection : IXmlable
	{
		private SerializableList<DocumentSetApplicationOptions> m_data;
		...
		public SerializableList<DocumentSetApplicationOptions> Data
		{
			get
			{
				return this.m_data;
		...
		public DocumentSetApplicationOptions this[string sContentTypeName]
		{
			get
			{
				DocumentSetApplicationOptions documentSetApplicationOption;
				using (IEnumerator<DocumentSetApplicationOptions> enumerator = this.m_data.GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						DocumentSetApplicationOptions current = enumerator.Current;
						if (current.ContentTypeName != sContentTypeName)
						{
							continue;
						}
						documentSetApplicationOption = current;
						return documentSetApplicationOption;
					}
					return null;
				}
				return documentSetApplicationOption;
		...
		public DocumentSetApplicationOptions this[DocumentSetApplicationOptions key]
		{
			get
			{
				DocumentSetApplicationOptions documentSetApplicationOption;
		...
		public DocumentSetApplicationOptionsCollection()
		{
			this.m_data = new CommonSerializableList<DocumentSetApplicationOptions>();
		}

		public DocumentSetApplicationOptionsCollection(DocumentSetApplicationOptions[] items)
		{
			this.m_data = new CommonSerializableList<DocumentSetApplicationOptions>(items);
		}

		public DocumentSetApplicationOptionsCollection(XmlNode node)
		{
			this.m_data = new CommonSerializableList<DocumentSetApplicationOptions>();
			this.FromXML(node);
		}
		...
		public void ApplyContentTypeMappingsToListItem(XmlNode itemXml, SPListItem sourceItem, SPList targetList)
		{
			...
			foreach (DocumentSetApplicationOptions mDatum in this.m_data)
			{
				if (!mDatum.ItemIsMapped(sourceItem))
				{
					continue;
				}
				documentSetApplicationOption = mDatum;
				break;
			}
			if (documentSetApplicationOption == null)
			{
				return;
			}
			...
			{
				if (contentType.Name != documentSetApplicationOption.ContentTypeName)
				{
					continue;
				}
		...
		public DocumentSetApplicationOptionsCollection Clone()
		{
			return new DocumentSetApplicationOptionsCollection(XmlUtility.StringToXmlNode(this.ToXML()));
		}

		public void FromXML(XmlNode xmlNode)
		{
			XmlNode xmlNodes = xmlNode.SelectSingleNode(".//DocumentSetApplicationOptionsCollection");
		...
		{
			xmlWriter.WriteStartElement("DocumentSetApplicationOptionsCollection");
